// Exercise 5.6
// Rewrite the extract_fails function from §5.1.1/77 so that instead of erasing each failing student from the input
// vector v, it copies the records for the passing students to the beginning of v, and then uses the resize function to
// remove the extra elements from the end of v. How does the performance of this version compare with the one in
// §5.1.1/77?
...
std::vector<Student> extract_fails(std::vector<Student>& students)
{
    ...
    {
        iterPassed = std::find_if(iterPassed, students.end(), fgrade);
...
std::vector<Student> extract_fails2(std::vector<Student>& students)
{
    std::vector<Student>::iterator iterPassed = students.begin();
    std::vector<Student>::iterator iter = students.begin();
...
int main()
{
    ...
    const auto failed = extract_fails(students); // extract_fails2(students)
